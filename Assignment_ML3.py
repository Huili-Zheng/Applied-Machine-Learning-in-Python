...
def answer_three():
    ...
    return accuracy,recall,precision,y_pred
answer_three()
...
def answer_five1():
    ...
    plt.figure()
    #plt.subplot(1,2,1)
    pr= plt.plot(precision, recall,label='Recision Recall Curve')
    ...
    plt.axvline(0.16)
    plt.show()
    x_values = pr[0].get_xdata()
    y_values = pr[0].get_ydata()
    pre = np.where(x_values == 0.75)
    recall = y_values[pre]
    x_values1=roc[0].get_xdata()
    y_values1=roc[0].get_ydata()
    index_of_nearest_fpr = bisect.bisect_left(fpr_lr,0.16)
    tpr = tpr_lr[index_of_nearest_fpr]
    #tp = y_values1[]
...
def answer_six():    
    ...
    CV_result = grid_lr_rec.cv_results_
